"""
キャプション生成エージェント
Instagram Reel・Threads用のキャプションとハッシュタグを自動生成する

対応コンテンツタイプ：
  threads_product  → Threads投稿用キャプション（商品情報あり・URL誘導なし）
  threads_buzz     → Threads投稿用キャプション（共感・あるある系）
  reel             → Instagramキャプション（バイラルリール用）
"""
import anthropic
import os
import re
...
・【厳守】450字以内
・アフィリエイトURLは絶対に貼らない
・商品投稿の場合：最後に「詳しくは @babyboo のInstagramで🎬」と入れてInstagramへ誘導
・バズ・あるある投稿の場合：Instagramへの誘導は任意
...
def generate_caption(script: dict, platform: str, product: dict = None) -> str:
    """
    コンテンツタイプ・プラットフォーム別のキャプションを生成する。
    script["type"] で threads_product / threads_buzz / reel / reel_threads を判定する。
    """
    content_type = script.get("type", "reel")
    struggles_str = ", ".join(MONTHLY_STRUGGLES.get(MONTH_AGE, MONTHLY_STRUGGLES[4]))

    if content_type == "threads_product" and product:
        content_label = "Threads 商品投稿（さりげなく・体験談ベース）"
        context_info = f"""
商品名：{product['name']}
価格：¥{product['price']:,}
方向性：{script.get('caption_hint', '')}
※商品を「紹介する」のではなく「使った体験を話す」スタンスで書く
※アフィリエイトURLは絶対に貼らない。末尾にInstagram誘導を入れる。
"""
        closing = "最後に「詳しくはInstagramで」または「気になる人はInstagram (@aibaby.jp) でも書いてるよ」を自然に添える"
        bad_good = """
【NG例（AIっぽい・絶対使わない）】
× 「この商品は非常に高品質で育児に役立ちます」
× 「せなっちにぴったりの商品を見つけました！ぜひチェックしてみてください」
× 「育児グッズ紹介です。こちらのアイテムはとても便利です」

【OK例（リアルな体験談）】
○ 「昨日おくるみ買い替えたらせなっちが30分で寝てくれてマジか…ってなった」
○ 「スタイ変えてから1日の洗い物が半分になった気がする。なんで早く買わなかったんだろ」
○ 「夜泣きで疲れてた時にこれ使ったら違くて。高いけどまあ…あり」"""

    elif content_type == "threads_buzz":
        content_label = "Threads バズ・あるある投稿（育児パパのリアルな独り言）"
        context_info = f"""
テーマ：{script.get('theme', '')}
方向性：{script.get('caption_hint', '')}
今の月齢あるある：{struggles_str}
"""
        closing = "「わかる人いる？」「同じ月齢の人コメント欲しい」「これあるある」などのどれか"
        bad_good = """
【NG例（AIっぽい・絶対使わない）】
...
○ 「育休とった同期に「大変だった？」って聞いたら「意外と余裕」って言ってて意味わかんなかった。今ならわかる。無理。」"""
        ...
        # reel_threads = 動画をThreadsに投稿するときのキャプション（短め）
        content_label = "Threads 動画投稿キャプション（バイラルリール用）"
        product_info = f"（動画でせなっちが着ている・使っている商品：{product['name']}）" if product else ""
        context_info = f"""
動画コンセプト：{script.get('viral_concept', script.get('hook', ''))}
{product_info}
"""
        closing = "「フォローするとせなっちの成長が追えるよ」または「これ何使ってるんですか？って聞いていいよ笑」"
        bad_good = """
【NG例】× 「可愛い動画をお届けします」× 「せなっちの様子です」× 「商品を着用しています」
【OK例】○ 「着せたら動き出しちゃって笑」○ 「生後{MONTH_AGE}ヶ月でこんな声出んの？」○ 「これ着てるの見てほしくて」"""

    else:  # instagram reel caption
...
"""
        ...
        bad_good = """
【NG例】× 「せなっちの可愛い動画です」× 「この商品を使用しています」× 「商品紹介です」
【OK例】○ 「この反応見てパパ泣いた😭」○ 「生後{MONTH_AGE}ヶ月ってこんな声出んの？笑」○ 「着せたらこうなった」"""
...
def _trim_threads(caption: str, affiliate_url: str = "") -> str:
    """
    Threadsの500文字制限に収まるよう調整する。
    アフィリエイトURLは貼らない（アカウント制限リスク）。
    Instagramへの誘導文を維持しつつ450字以内に収める。
    """
    LIMIT = 450

    # 万が一アフィリエイトURLが含まれていたら除去
    if affiliate_url and affiliate_url in caption:
        caption = caption.replace(affiliate_url, "").strip()
    # hb.afl.rakuten や amazon のリンクが混入していたら除去
...
def run(scripts: list, product: dict = None) -> list:
    """
    3種類のコンテンツ（threads_product / threads_buzz / reel）に
    プラットフォーム別キャプションを生成する。

    - threads_product → captions["threads"]
    - threads_buzz    → captions["threads"]
    - reel            → captions["instagram"] + captions["threads"]（動画をThreadsにも投稿）
    """
    ...
    for i, script in enumerate(scripts):
        ...
        if content_type == "threads_product":
            # Threadsキャプション（商品情報あり）
            caption = generate_caption(script, platform="threads", product=product)
            caption = _trim_threads(
                caption,
                affiliate_url=product.get("affiliate_url", "") if product else ""
            )
            captions["threads"] = caption
            print(f"    threads用キャプション生成完了（{len(caption)}文字）")

        elif content_type == "threads_buzz":
            # Threadsキャプション（バズ・あるある系）
            ...
            # Threadsキャプション（同じ動画をThreadsにも投稿）
            ...
            print(f"    threads用キャプション生成完了（{len(caption_th)}文字）")
