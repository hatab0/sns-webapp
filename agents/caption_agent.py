...
def generate_youtube_shorts_content(script: dict, product: dict = None) -> dict:
    """
    YouTube Shorts用のタイトルと説明文を1回のAPI呼び出しで生成する。
    戻り値: {"title": str, "description": str}
    """
    product_name = product["name"] if product else ""
    hook = script.get("hook", "")
    concept = script.get("viral_concept", "")

    prompt = f"""
あなたはYouTube Shortsの育児チャンネル運営者です。
以下の動画コンテンツに最適なタイトルと説明文を生成してください。
...
【タイトルのルール】
・100文字以内（Shorts検索で引っかかる表現）
・「生後{MONTH_AGE}ヶ月」を入れる
・「〇〇したら△△だった」「〇〇が可愛すぎた」のような感情系タイトル
・商品名は入れすぎない（宣伝っぽくならないこと）
・例：「生後4ヶ月の赤ちゃんにおくるみ着せたら想像以上だった」

【説明文のルール】
・3〜5行程度の自然な文章
・「#Shorts」を必ず入れる
・末尾に以下のタグを全て入れる：{FIXED_TAGS_STR} #Shorts #生後{MONTH_AGE}ヶ月 #育児vlog #ベビーグッズ
...
"""
    ...
    try:
        ...
        return {"title": f"生後{MONTH_AGE}ヶ月せなっちの成長記録", "description": text}
...
def run(scripts: list, product: dict = None) -> list:
    ...
    - reel         → captions["instagram"] / captions["threads"] / captions["youtube_title"] / captions["youtube"]
    """
    ...
    for i, script in enumerate(scripts):
        ...
        if content_type == "threads_text":
            ...
            yt = generate_youtube_shorts_content(script, product=product)
            captions["youtube_title"] = yt.get("title", "")
            captions["youtube"] = yt.get("description", "")
            print(f"    YouTube Shorts用コンテンツ生成完了")
