...
def _base_scene_text_buzz() -> str:
    """バズmode：baby_cubo_officialスタイルの月齢特化シーン（2枚参照画像前提）"""
...
【前提：ユーザーから2枚の参照画像が送信されます】
  参照画像1: 赤ちゃん「せなっち」の写真 → 顔・外見の完全一致に使用（目・鼻・輪郭・肌色）
  参照画像2: コスチュームの画像 → 着せる服装として使用（デザイン・色・形を完全一致）

【生後{MONTH_AGE}ヶ月のせなっちに特化したシーン指示】

ポーズ：{scene['pose']}

表情（最重要）：{scene['expression']}
→ 誇張しすぎず、「本当に撮れた一瞬」のような自然なリアルさで描写すること

カメラ・構図：{scene['camera']}
→ 1:1 square format / 85mm portrait lens / very shallow depth of field

照明：{scene['lighting']}
→ 目に必ずキャッチライト（白い光の点）を入れること。これが「生き生きした目」の鍵

背景：{scene['background']}
→ 背景はできる限りシンプル・ボケさせて赤ちゃんを際立たせる

コスチューム（参照画像2に合わせるが、月齢イメージ参考）：{costume}

【撮影品質の指示（必須）】
・Style: Hyper-realistic professional infant photography, editorial quality
・Skin: Smooth, soft, naturally glowing baby skin — not plastic or airbrushed
・Eyes: Tack sharp with prominent natural catchlights, bright and alive
・Overall: Warm, slightly golden color grade, true-to-life colors
