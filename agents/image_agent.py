...
_last_buzz_selections: dict = {}  # 直近生成の選択内容（成果計測との紐付け用）
...
def _base_scene_text_buzz(scene: dict, event: dict = None) -> str:
    ...
    # 成果計測（post_metrics）と紐付けるため直近の選択を保持
    _last_buzz_selections = {
        "costume": costume, "background": background,
        "accessory": accessory, "hat": standalone_hat,
    }
...
def run_buzz(event: dict = None, today_scene: str = "") -> dict:
    ...
    image_prompt = generate_image_prompt_notxt_buzz(scene, event=event, today_scene=today_scene)
    result = {
        ...
        "item_code": "",
        "gpt_image_prompt": "",
        "gpt_image_prompt_notxt": image_prompt,
        "video_prompt": generate_kling_prompt_buzz(scene, event=event, today_scene=today_scene),
        "buzz_scene": today_scene or scene.get("pose", ""),
        "buzz_selections": dict(_last_buzz_selections),
        "room_description": "",
        "score": 0,
        "is_buzz_mode": True,
        "event": event,
